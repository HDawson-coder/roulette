...
public interface SpinDao {
  ...
  @Delete
  Single<Integer> delete (Spin spin); // a single spin

  @Delete
  Single<Integer> delete (Spin... spin); // a whole bunch of spins

  @Delete
  Single<Integer> delete(Collection<Spin> spins); // a collection of spins
  ...
  @Query("SELECT s.*, t.total_wager, t.total_payout "
      + "FROM Spin AS s LEFT JOIN "
      + "(SELECT w.spin_id, SUM(w.amount) as total_wager, SUM(w.payout) as total_payout "
      + "FROM Wager AS w GROUP BY w.spin_id) AS t ON s.spin_id = t.spin_id "
